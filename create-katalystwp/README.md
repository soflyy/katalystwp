...
Update refreshes only Katalyst-owned tooling files (`scripts/`, Dockerfiles, compose file, README, skills, the menu). Before doing anything it explains exactly what will be replaced, recommends a backup (a git commit or a copy of the folder), and asks for confirmation — `--yes` skips the question for automation. It never touches your WordPress site, database, wp-content, `.env`, `sandbox.config.json` settings (only the recorded `scaffolderVersion` is stamped), `php/php.ini`, or `scripts/user-setup.sh` / `scripts/dev.sh` — and npm scripts you added to `package.json` are preserved. If you hand-edited a Katalyst-owned file (say, `docker-compose.yml`), those edits are what the backup is for. Container-level changes apply on the next `npm run setup` (idempotent — it does **not** reinstall WordPress; already-provisioned steps are skipped).
